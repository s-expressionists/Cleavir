     ...
     (lambda (state item index)
       (case state
         ...
         ((&rest)
          ...
          (let ((consp nil) (nullp nil) (top (ctype:top system)))
            (dolist (argstype argstypes)
              (let ((nreq (length (ctype:values-required argstype system)))
                    (nopt (length (ctype:values-optional argstype system)))
                    (rest (ctype:values-rest argstype system)))
                (cond ((< index nreq) (setf consp t))
                      ((< index (+ nreq nopt))
                       (setf consp t nullp t))
                      ((ctype:bottom-p rest system) (setf nullp t))
                      (t (setf consp t nullp t)))))
            (setf (bir:derived-type item)
                  (ctype:single-value
                   (cond ((and consp nullp)
                          (ctype:disjoin/2 (ctype:member system nil)
                                           (ctype:cons top top system)
                                           system))
                         (nullp (ctype:member system nil))
                         (consp (ctype:cons top top system))
                         (t bottom))
                   system))))
