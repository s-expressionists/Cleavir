...
;; NOTE: This is never actually called with an instruction of ENCLOSE,
;; so this is ok.
(defmethod remove-use ((datum function) (use instruction))
...
(defmethod add-use ((datum function) (use instruction))
...
(defmethod clean-up-instruction progn ((inst enclose))
  (let ((code (code inst)))
    ...
    (when (and (set:empty-set-p (local-calls code))
               (set:empty-set-p (other-uses code)))
...
(defmethod clean-up-instruction progn ((inst abstract-local-call))
  (let* ((code (callee inst))
    ...
    (when (and (null (enclose code))
               ...
               (set:empty-set-p (other-uses code)))
      (clean-up-function code))))
(defmethod clean-up-instruction progn ((inst unwind-protect))
  (let* ((code (input inst))
         (other-uses (other-uses code)))
    (set:nremovef other-uses inst)
    (when (and (null (enclose code))
               (set:empty-set-p (local-calls code))
               (set:empty-set-p other-uses))
      (clean-up-function code))))
