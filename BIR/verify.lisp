...
(defmethod verify-outputs ((c catchi))
  (test (member (length (outputs c)) '(0 1))
        "has more than one output ~a" (outputs c))
  (when (= 1 (length (outputs c)))
    (let ((output (first (outputs c))))
      (test (typep output 'phi)
            "has non-phi output ~a" c output)
      (test (set:presentp c (definitions output))
            "is not a definition of its output ~a" c output)
      (let* ((aib (second (next c))) (inputs (inputs aib)))
        (test (and (= (length inputs) 1) (eq (first inputs) output))
