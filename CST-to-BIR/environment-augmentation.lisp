...
(defun ignore-from-decls (raw-declarations)
  (cond ((member 'ignore raw-declarations :test #'eq :key #'car)
         'ignore)
        ((member 'ignorable raw-declarations :test #'eq :key #'car)
         'ignorable)
        (t nil)))
...
(defun augment-environment-with-variable
  ...
  (let* ((new-env env)
         ...
         (ignore (ignore-from-decls raw-declarations)))
      ...
      (if special-p
          ...
          (let ((lexical-variable
                  (make-instance 'cleavir-bir:variable
                    :name raw-variable :ignore ignore)))
            (setf new-env
...
(defun augment-environment-with-local-function-name (name-cst identity
                                                     environment)
  (let ((name (cst:raw name-cst)))
    (env:add-local-function environment name identity)))
